...
export default function NavUser() {  
  ...
  const [groupName,setGroupName]= useState('');
    ...
    // verifer s'il a un groupe pour ajouter les infos
    db.collection('User').doc(auth?.currentUser?.uid).onSnapshot(doc => setIdGroup(doc.data().idGroup));
    useLayoutEffect(() =>{
        console.log(IdGroup);
        let idGroup = localStorage.getItem('idGroup');
          if(idGroup === ""){
            console.log(" il n'a pas de groupe");
            }
            else{
                //recuperation de mes infos dans mon store
               let groupName = localStorage.getItem('GroupName');
             let logoUrl= localStorage.getItem('GroupLogoUrl');
    ...
    useEffect(() =>{
         setUserName (currentUser?.displayName);// je vais verifier si il a ete enregistre avec sonn gmail
         if(currentUser?.photoURL){
            setphotoUrl(currentUser?.photoURL)
         }else{
            storage.ref('/user/'+ currentUser?.uid +'/profileImg' ).getDownloadURL().then(imgUrl =>{
                setphotoUrl(imgUrl);
            }).catch(err => console.log(err.message))
            // et je lui attribue sa phto
         } 
          // s'l a ete enregistre par mon systeme de notifiaction
    ...
    return (
        <>
                ...
                {/* Code block starts */}
                <nav className="w-full bg-blue-900 hidden xl:block shadow">
                    <div className="container px-6 h-16 flex justify-between items-center lg:items-stretch mx-auto">
                        <div className="flex items-center">
                            <div className="mr-10 flex items-center">
                                {groupLogo ? <img src={groupLogo} className="object-container w-24 h-12" /> :  <svg aria-label="Home" id="logo" enableBackground="new 0 0 300 300" height={44} viewBox="0 0 300 300" width={43} xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
                                    <g>
                                        <path
                                            fill="#4c51bf"
                                ...
                                <h3 className="text-base text-white font-bold tracking-normal leading-tight ml-3 hidden lg:block">{groupName ? groupName : "Nom du groupe"}</h3>
                            </div>
                            <ul className="hidden xl:flex items-center h-full">
                                <li className="cursor-pointer h-full flex items-center text-sm text-indigo-700 tracking-normal transition duration-150 ease-in-out">
                                    ...
                                    <Link to="/group">parametres de groupe</Link>
                                </li>
                                ...
                                <li className="cursor-pointer h-full flex items-center text-sm hover:text-indigo-700 text-white mr-10 tracking-normal transition duration-150 ease-in-out">
                                    ...
                                    <Link to='/update'>modifier informations</Link>
                                    
                                </li>
                                <li onClick={logout} className="cursor-pointer h-full flex items-center text-sm hover:text-indigo-700 text-white tracking-normal transition duration-150 ease-in-out">
                                    <span className="mr-2">
                        ...
                        <div className="h-full hidden xl:flex items-center justify-end">
                            <div className="h-full flex">
                                <div className="px-6 h-full  justify-center border-l border-gray-700 text-gray-400 flex items-center">
                                ...
                                <div className="flex items-center pl-8 relative cursor-pointer" onClick={() => setProfile(!profile)}>
                                    ...
                                    <img className="rounded h-10 w-10 object-cover" src={photoUrl} alt="logo" />
                                    <p className="text-white text-sm ml-2">{userName}</p>
                                </div>
                            </div>
                        </div>
                            ...
                            <ul className="p-2 border-r bg-white absolute rounded top-0 left-0 right-0 shadow mt-16 md:mt-16 hidden">
                                <li className="flex md:hidden cursor-pointer text-gray-600 text-sm leading-3 tracking-normal mt-2 py-2 hover:text-indigo-700 focus:text-indigo-700 focus:outline-none">
                                    <div className="flex items-center">
                                        ...
                                        <span className="ml-2 font-bold">Poster un projet</span>
                                    </div>
                                </li>
                                <li className="flex md:hidden flex-col cursor-pointer text-gray-600 text-sm leading-3 tracking-normal py-2 hover:text-indigo-700 focus:text-indigo-700 focus:outline-none  justify-center" onclick="dropdownHandler(this)">
                                    <div className="flex items-center">
                                        ...
                                        <span className="ml-2 font-bold"><Link to="/group">parametres de groupe</Link></span>
                                    </div>
                                </li>
                                <li className="flex md:hidden cursor-pointer text-gray-600 text-sm leading-3 tracking-normal py-2 hover:text-indigo-700  items-center focus:text-indigo-700 focus:outline-none">
                                    ...
                                    <span className="ml-2 font-bold"><Link to='/update'>modifier informations</Link></span>
                                </li>
                                <li onClick={logout} className="border-b border-gray-300  md:hidden flex-col cursor-pointer text-gray-600 text-sm leading-3 tracking-normal pt-2 pb-4 hover:text-indigo-700 focus:text-indigo-700 focus:outline-none flex justify-center" onclick="dropdownHandler(this)">
                                    <div className="flex items-center">
                                        <svg xmlns="http://www.w3.org/2000/svg" className="icon icon-tabler icon-tabler-code" width={20} height={20} viewBox="0 0 24 24" strokeWidth="1.5" stroke="currentColor" fill="none" strokeLinecap="round" strokeLinejoin="round">
                                            <path stroke="none" d="M0 0h24v24H0z" />
                                            <polyline points="7 8 3 12 7 16" />
                                        ...
                                        <span className="ml-2 font-bold">se deconnecter</span>
                                    </div>
                                </li>
                                <li className="cursor-pointer text-gray-600 text-sm leading-3 tracking-normal mt-2 py-2 hover:text-indigo-700 flex items-center focus:text-indigo-700 focus:outline-none">
                                    <div className="flex items-center">
                                ...
                                <div className="flex flex-col justify-between h-full w-full">
                                    ...
                                    <g>
                                        ...
                                        />
                                                    ...
                                                    <p className="text-base  text-white ml-3">{groupName ? groupName : 'nom du groupe'}</p>
                                                </div>
                                            ...
                                            <a className="cursor-pointer">
                                                <li className="text-white pt-10">
                                                    <div className="flex items-center">
                                                        ...
                                                        <p className="text-indigo-700 xl:text-base text-base ml-3">Poster un projet</p>
                                                    </div>
                                                </li>
                                            </a>
                                            <a className="cursor-pointer">
                                                <li className="text-white pt-8">
                                                    <div className="flex items-center justify-between">
                                                        <div className="flex items-center">
                                                            ...
                                                            <p className="text-white xl:text-base  text-base ml-3"><Link to="/group">parametres de groupe</Link></p>
                                                        </div>
                                                    </div>
                                                </li>
                                            </a>
                                            <a className="cursor-pointer">
                                                <li className="text-white pt-8">
                                                    <div className="flex items-center">
                                                        ...
                                                        <p className="text-white xl:text-base  text-base ml-3"><Link to='/update'>modifier informations</Link></p>
                                                    </div>
                                                </li>
                                            </a>
                                            <li onClick={logout} className="text-white pt-8 cursor-pointer">
                                                <div className="flex items-center justify-between">
                                                    <div className="flex items-center">
                                                        <div className="w-6 h-6 md:w-8 md:h-8 text-white">
                                                        ...
                                                        <p className="text-white xl:text-base  text-base ml-3">Se deconnecter</p>
                                                    </div>
                                                </div>
                                            </li>
                                        </ul>
                                    ...
                                    <div className="w-full pt-4">
                                        <div className="flex justify-center mb-4 w-full">
                                            <div className="relative w-full">
                                                ...
                                                <input className=" focus:outline-none rounded w-full text-sm text-gray-500 bg-gray-700 pl-10 py-2" type="text" placeholder="Search" />
                                            </div>
                                        </div>
                                        <div className="border-t border-gray-700">
                                            <div className="w-full flex items-center justify-between pt-1">
                                                <div className="flex items-center">
                                                    <img alt="profile-pic" src={photoUrl} className="w-8 h-8 rounded-md" />
                                                    <p className=" text-white text-base leading-4 ml-2">{userName}</p>
                                                </div>
                                                <ul className="flex">
